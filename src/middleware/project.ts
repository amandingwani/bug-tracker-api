...
export const removeContributorMiddleware = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (
      ...
      (existingProject.ownerId === res.locals.userData.id ||
        ...
        existingProject.contributors.find(
          (c) => c.id === res.locals.userData.id && c.email === res.locals.parsedData.email
        ))
    ) {
      next();
    } else {
